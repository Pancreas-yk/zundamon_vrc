...
impl EventProcessor {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        while let Ok((window, event)) = self.ime_event_receiver.try_recv() {
            ...
            tracing::debug!("[IME] drain_ime_events: {:?}", event);
            let event = match event {
                ...
                ImeEvent::Update(text, position) => {
                    // Some XIM implementations (e.g. fcitx5) may send
                    // preedit_draw without a preceding preedit_start for
                    // compositions after the first one.  Treat the Update
                    // itself as proof that we are composing.
                    self.is_composing = true;
                    WindowEvent::Ime(Ime::Preedit(text, Some((position, position))))
                },
                ImeEvent::End => {
                    // NOTE: Do NOT set is_composing = false here.
                    // The committed text arrives as a subsequent key event
                    // that must go through the lookup_utf8 (Commit) path.
                    // If we clear is_composing now, that key event is
                    // misrouted through the KeyboardInput path instead,
                    // and the Commit is never emitted.
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
            ...
            wt.ime.as_ref().and_then(|ime| ime.borrow().get_context(window as XWindow))
        {
            ...
            if !written.is_empty() {
                ...
                // the composing flag so normal keyboard input resumes.
                self.is_composing = false;
            }
        }
    }
