...
extern "C" fn preedit_start_callback(
    ...
    tracing::debug!("[IME] preedit_start_callback fired");
...
extern "C" fn preedit_done_callback(
    ...
    tracing::debug!("[IME] preedit_done_callback fired");
...
extern "C" fn preedit_draw_callback(
    ...
    tracing::debug!(
        "[IME] preedit_draw_callback: caret={}, chg_first={}, chg_length={}",
        call_data.caret,
        call_data.chg_first,
    ...
    let preedit_text: String = client_data.text.iter().collect();
    tracing::debug!("[IME] preedit_draw Update: text={:?}, cursor_byte_pos={}", preedit_text, cursor_byte_pos);

    client_data
        .event_sender
        .send((
            client_data.window,
            ImeEvent::Update(preedit_text, cursor_byte_pos),
        ))
...
impl ImeContext {
    pub(crate) unsafe fn new(
        ...
        tracing::info!("[IME] ImeContext::new: allowed={}, negotiated={:?}", allowed, style);

        // When IME is allowed but the server only advertised Nothing/None
        // style, try creating a Preedit (on-the-spot) XIC first.  Many XIM
        // servers (e.g. fcitx5) actually support on-the-spot preedit
        // callbacks even when they don't advertise it; this gives us inline
        // preedit text in the application instead of a separate floating
        // window.
        let ic = if allowed && !matches!(style, Style::Preedit(_)) {
            let preedit_style_value =
            ...
            if maybe_ic.is_some() {
                tracing::info!("[IME] forced on-the-spot (PreeditCallbacks) XIC creation succeeded");
                maybe_ic
            } else {
                tracing::info!("[IME] forced PreeditCallbacks XIC failed, falling back to {:?}", style);
                // Clear X errors from the failed attempt.
                let _ = xconn.check_errors();
                match style as _ {
                    Style::Nothing(s) => unsafe {
                        ImeContext::create_nothing_ic(xconn, im.im, s, window)
                    },
                    Style::None(s) => unsafe {
                        ImeContext::create_none_ic(xconn, im.im, s, window)
                    },
                    Style::Preedit(_) => unreachable!(),
                }
            }
        } else {
            match style as _ {
